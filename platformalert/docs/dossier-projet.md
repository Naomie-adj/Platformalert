...
| BC02 | CP2 — Stockage | Volumes PostgreSQL Docker | Section 8 |
...
├── Build image API (./api)
└── Build image Scraper (./scraper)
│
▼
aquasecurity/trivy-action@v0.35.0
├── Scan API — 0 vulnérabilité CRITICAL/HIGH
└── Scan Scraper — 0 vulnérabilité CRITICAL/HIGH
│
▼
Push DockerHub
...
- Ajouter AWS Backup pour la stratégie de sauvegarde PostgreSQL
- Implémenter un frontend React pour visualiser les prix
